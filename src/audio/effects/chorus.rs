...
impl ChorusEffect {
    pub fn new() -> Self {
        let max_delay = (0.05 * 96000.0) as usize;
        Self {
            params: vec![
                ParameterValue::new(0.5),
                ParameterValue::new(0.5),
                ParameterValue::new(0.4),
            ],
            info: vec![
                ParameterInfo { id: 0, name: "Rate".into(),      label: "Hz".into(), min_value: 0.1, max_value: 5.0, default_value: 0.5, flags: ParameterFlags::default() },
                ParameterInfo { id: 1, name: "Depth".into(),     label: "".into(),    min_value: 0.0, max_value: 1.0, default_value: 0.5, flags: ParameterFlags::default() },
                ParameterInfo { id: 2, name: "Mix".into(),       label: "".into(),    min_value: 0.0, max_value: 1.0, default_value: 0.4, flags: ParameterFlags::default() },
            ],
...
impl DspEffect for ChorusEffect {
    fn process(&mut self, input_l: &mut [f32], input_r: &mut [f32], _sample_rate: u32) {
        ...
        for i in 0..input_l.len() {
            ...
            let mod_delay = (lfo * 0.5 + 0.5) * depth * max_delay * 0.5;
    ...
    fn reset(&mut self, sample_rate: u32) {
        ...
        self.write_pos = 0;
        self.phase = 0.0;
        let max_delay = (0.05 * sample_rate as f32) as usize;
